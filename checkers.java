...
public class checkers {
  ...
  */ 
                                  ...
                                  if (positions[cord2-1][cord1-1] != cord) {
                                      switch (positions[cord2-1][cord1-1] + 1) {
                                          ...
                                          case 1:
                                              ...
                                              legalcheck++;
                                              tstring += letters.charAt(cord1-1); 
                                              tstring += numbers.charAt(cord2-1);
                                              break;
                                      }    
              ...
              if (legalcheck > 0 || robotturn == 1) {
                  ...
                  while (inputCondition==0) {
                      if (robotturn == 0) {
                          ...
                          // The program still displays the same lines but does not read in text when the computer plays
                          System.out.println("Enter target coordinate");
                      ...
                      int intcheck = 0;
                      for (int num=0;num<tstring.length();num++) { // Determine if a piece will be on edge of the board
                      ...
                      switch (intcheck) {
                          case 1: // This will promote a normal piece into a king piece if it reaches an end of the board
                              ...
                              break;
                          case 2: //This will move a piece on the board
                              ...
                              // Check and perform jumps over pieces if jump conditions are met
                              Boolean done = false;
                              while (done==false) {
                              ...
                              break;
                          default: // If user entered an invalid target coordinate
  ...
  public String checkMoves(String incoming) { //This method will take in the user's input to update the checker board and piece locations
	  // Take in the values of the user's input for the target coordinate
      ...
      if (cord == 2) { // Makes a move if the piece selected belongs to player 2 (It is player 2's turn)
          if (cord == 2) {
              switch (cord2) { // This is to move a piece accordingly to prevent an out of bounds error (when selected piece is already on edge of ther board)
              	  // For a piece between rows 2-7
                  case 1:
                  case 2:
                  case 3:
                  case 4:
                  ...
                  case 6:
                      ...
                      break;
                  case 0: // Makes a move for a piece on the top edge of the board (row 1)
                      if (positions[cord2+1][cord1-1] != cord) {
                          switch (positions[cord2+1][cord1-1] + 1) {
                              case 2:
                                  if ((cord2+1) < 7 && cord1-1 > 0) {
                      ...
                      break;
                  case 7: // Makes a move for a piece on the bottom edge of the board (row 8)
                      if (positions[cord2-1][cord-1] != cord) {
                          switch (positions[cord2-1][cord1-1] + 1) {
                              case 2:
                                  if ((cord2-1) > 0 && cord1-1 > 0) {
      ...
      } else if (cord == 1) { // Makes a move if the piece selected belongs to player 1 (It is player 1's turn)
          if (cord==1) { 
              switch (cord2) { // This is to move a piece accordingly to prevent an out of bounds error (when selected piece is already on edge of ther board)
          	  // For a piece between rows 2-7
                  case 1:
    	  ...
    	  /*
    	   * Makes a move if the piece selected is a king piece belonging to either player 1 or 2
    	   *  (It is either player's turn)
    	   */
          if (cord1-1 >= 0) {
              if (cord2-1 >= 0) {
                  if (positions[cord2-1][cord1-1] != cord) {
      ...
      // This will run the 'brain' for the computer if computer mode is on
      // It will make the computer play the best move on its turn
      if (legalcheck > 0) {
          ...
          for (int row=0;row<4;row++) {
              ...
              // Computer will take in the current board positions
              ...
              switch (intcheck) {
                  case 2: // Computer can only move Player 2 pieces
                      ...
                      while (done==false) {
                          ...
                          // These if statements will calculate potential moves for the computer and 
                          // records the score (Number of jumps. A move with more jumps available is a better move
                          if (cord3-cord1 > 0 || cord > 2) {                              
                                  if (cord6<6 && cord5<6) {
                          ...
                          if (jumped == false) { // End loop when no more piece jumps are available
                              done = true;
                          }
                      }
                      break;
                  default: // If the piece isn't owned by the computer (Player 2 pieces)
                      legalCheck2=1;check=1;
                      break;
              }
      ...
      return (output); // Return the move and its score (number of jumps)
  }
}
